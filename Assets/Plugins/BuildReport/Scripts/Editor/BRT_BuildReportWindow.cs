	...
	void OnGUI()
	{
		...
		if ((_selectedCategoryIdx == USED_ASSETS_IDX && _buildInfo.HasUsedAssets) || (_selectedCategoryIdx == UNUSED_ASSETS_IDX && _buildInfo.HasUnusedAssets))
		{
			...
			GUILayout.BeginHorizontal();
				...
				string selectedInfoLabel = SELECTED_QTY_LABEL + assetListUsed.GetSelectedCount().ToString("N") + ". " + SELECTED_SIZE_LABEL + assetListUsed.GetReadableSizeOfSumSelection() + ". " + SELECTED_PERCENT_LABEL + assetListUsed.GetPercentageOfSumSelection().ToString("N") + "%";
